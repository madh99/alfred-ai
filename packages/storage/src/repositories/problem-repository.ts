...
export class ProblemRepository {
  ...
  async appendAnalysisNotes(userId: string, id: string, note: string): Promise<void> {
    ...
    await this.db.execute(
      `UPDATE cmdb_problems SET analysis_notes = CASE WHEN analysis_notes IS NULL OR analysis_notes = '' THEN ? ELSE analysis_notes || ? END, updated_at = ? WHERE id = ? AND user_id = ?`,
      [entry, `\n---\n${entry}`, now, id, userId],
    );
  }
