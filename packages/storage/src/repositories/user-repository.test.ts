...
describe.skipIf(!hasBetterSqlite3)('UserRepository', () => {
  ...
  async function setup() {
    ...
    const repo = new UserRepository(db.getAdapter());
    return repo;
  }
  ...
  it('should create a user via findOrCreate', async () => {
    ...
    const user = await repo.findOrCreate('telegram', 'tg-user-1', 'alice', 'Alice W');
  ...
  it('should find existing user', async () => {
    ...
    const first = await repo.findOrCreate('discord', 'dc-user-1', 'bob');
    const second = await repo.findOrCreate('discord', 'dc-user-1', 'bob');
  ...
  it('should update user', async () => {
    ...
    const user = await repo.findOrCreate('telegram', 'tg-user-2', 'charlie');
    expect(user.username).toBe('charlie');

    await repo.update(user.id, { username: 'charlie_updated' });
