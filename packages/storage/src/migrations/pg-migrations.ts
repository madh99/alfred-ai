...
export const PG_MIGRATIONS: PgMigration[] = [
  ...
  {
    version: 116,
    description: 'v929 — Interessen-Radar: interest_topics, topic_sources, topic_items, topic_digests (PG-Spiegel zu SQLite v112).',
    async up(db) {
      await db.execute(`CREATE TABLE IF NOT EXISTS interest_topics (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
        name TEXT NOT NULL,
      ...
      await db.execute(`CREATE INDEX IF NOT EXISTS idx_interest_topics_user ON interest_topics(user_id, status)`, []);
      await db.execute(`CREATE TABLE IF NOT EXISTS topic_sources (
        id TEXT PRIMARY KEY,
      ...
      await db.execute(`CREATE INDEX IF NOT EXISTS idx_topic_sources_topic ON topic_sources(topic_id, enabled)`, []);
      await db.execute(`CREATE TABLE IF NOT EXISTS topic_items (
        id TEXT PRIMARY KEY,
        topic_id TEXT NOT NULL,
        title TEXT NOT NULL,
        url TEXT,
      ...
      await db.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_topic_items_dedupe ON topic_items(topic_id, dedupe_hash)`, []);
      await db.execute(`CREATE INDEX IF NOT EXISTS idx_topic_items_topic ON topic_items(topic_id, created_at)`, []);
      await db.execute(`CREATE TABLE IF NOT EXISTS topic_digests (
  ...
  {
    version: 119,
    description: 'v993 — Redaktionsleitung: stories + story_assignments + content_items.story_id (PG-Spiegel zu SQLite v115).',
    async up(db) {
      ...
      await db.execute(`CREATE INDEX IF NOT EXISTS idx_stories_family ON stories(user_id, family, status)`, []);
      await db.execute(`CREATE TABLE IF NOT EXISTS story_assignments (
        id TEXT PRIMARY KEY,
      ...
      await db.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_story_assignments_dedupe ON story_assignments(story_id, channel_id)`, []);
