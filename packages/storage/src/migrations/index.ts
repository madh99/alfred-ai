...
export const MIGRATIONS: Migration[] = [
  ...
  {
    version: 95,
    description: 'v779 — agent_sessions: persistente CLI-Coding-Agent-Sessions pro Sandbox×Agent für tool-cache-retention zwischen Iterationen',
    up(db) {
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_agent_sessions_sandbox ON agent_sessions(sandbox_id)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_agent_sessions_last_used ON agent_sessions(last_used_at DESC)`);
  ...
  {
    version: 99,
    description: 'v823 — project conventions (CLAUDE.md/AGENTS.md) Auto-Generate + Drift + Lessons + Patterns + Violations + Test-Runs (alle Phasen 1-4 atomar).',
    up(db) {
      // Phase 1 — Primary table. Eine Row pro (project_id, package_path).
      // package_path = NULL bedeutet "root-level" (Phase 1 only). Mit Phase 3.1
      // Monorepo-Support gibt's pro Package eigene Rows.
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_conv_project ON agent_conventions(project_id)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_conv_drift ON agent_conventions(drift_score DESC)`);

      // Phase 3.2 — Audit-Log für Auto-Apply + Rollback.
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_conv_hist_project ON agent_conventions_history(project_id, package_path, applied_at DESC)`);

      // Phase 3.3 — Cross-Project Pattern Pool. Lokal pro masterUserId.
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_conv_patterns_user ON convention_patterns(master_user_id, retired_at)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_conv_patterns_occur ON convention_patterns(occurrence_count DESC)`);
      ...
      // Phase 4.2 — Inverse-Learning: per-Convention-Health tracking.
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_conv_violations_project ON convention_violations(project_id, package_path, violated_at DESC)`);

      // Phase 4.6 — Test-Harness Runs für A/B-Effectiveness-Tracking.
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_conv_testruns_task ON convention_test_runs(canonical_task_id, ran_at DESC)`);
  ...
  {
    version: 101,
    description: 'v847 — project_chat_actions: tracking-table für Chat-getriggerte Skill-Arbeit (D).',
    up(db) {
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_chat_actions_project ON project_chat_actions(project_id, started_at DESC)`);
  ...
  {
    version: 105,
    description: 'v866 — cli_agent_runs: separates Usage-Tracking für CLI-Agents (eigene Subscriptions/Keys, NICHT in llm_usage; SQLite-Spiegel zu PG v109).',
    up(db) {
      db.exec(`
      ...
      `);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_cli_runs_user ON cli_agent_runs(user_id, started_at DESC)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_cli_runs_project ON cli_agent_runs(project_id)`);
  ...
  {
    version: 111,
    description: 'v928 — insight_category_prefs: Kategorie-Mute („solche Insights nicht mehr"). Gemutete Kategorien erzeugen keine neuen Insight-Kandidaten (SQLite-Spiegel zu PG v115).',
    up(db) {
      db.exec(`CREATE TABLE IF NOT EXISTS insight_category_prefs (
        user_id TEXT NOT NULL,
        category TEXT NOT NULL,
  ...
  {
    version: 113,
    description: 'v933 — Social-Media-Betrieb: social_channels (Kanäle je Plattform, optional projekt-gebunden, Modus suggest|approve|autonomous), content_items (Pipeline idea→draft→scheduled→approved→published), channel_metrics (Performance für den Lern-Loop) (SQLite-Spiegel zu PG v117).',
    up(db) {
      db.exec(`CREATE TABLE IF NOT EXISTS social_channels (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
        project_id TEXT,
        platform TEXT NOT NULL,
      ...
      db.exec(`CREATE INDEX IF NOT EXISTS idx_social_channels_user ON social_channels(user_id, status)`);
      ...
      db.exec(`CREATE INDEX IF NOT EXISTS idx_content_items_channel ON content_items(channel_id, status)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_content_items_sched ON content_items(status, scheduled_at)`);
      db.exec(`CREATE TABLE IF NOT EXISTS channel_metrics (
        id TEXT PRIMARY KEY,
        channel_id TEXT NOT NULL,
        item_id TEXT,
        date TEXT NOT NULL,
      ...
      // COALESCE: NULLs sind in Unique-Indizes distinct — Kanal-Metriken (item_id NULL)
      // würden sonst nicht dedupliziert
