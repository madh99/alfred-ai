...
const CATEGORY_KEYWORDS: Record<Exclude<SkillCategory, 'core'>, RegExp> = {
  ...
  identity:     /\b(link|verknüpf|cross.?platform|identity|identität|user|benutzer|nutzer|rolle|role|invite|einlad|connect|verbinde?|whoami|wer\s+bin\s+ich|profil|registrier|onboard|deaktiv|aktivier|einrichte?n?|richte?\s+.*\s+ein|konfigur\w*|setup.?service|postfach|account|microsoft|outlook|o365|office.?365|auth.?microsoft|sende\s+\w+\s+(per|auf|via|über)|nachricht\s+an|schreib\w*\s+\w+\s+(per|auf|eine?\s+nachricht))/i,
  mcp:          /\bmcp\b/i,
};
